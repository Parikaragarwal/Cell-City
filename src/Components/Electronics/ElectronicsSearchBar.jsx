...
import Loader from '../Loader'; // Keep this as-is

const ElectronicsSearchBar = ({
  products = [],
  onSearchResults,
  placeholder = "Search electronics by brand, name, type, or specs...",
  className = ""
}) => {
  ...
  const performSearch = (term) => {
    ...
    const results = products.filter((product) => {
      return (
        ...
        product.specs.some(s =>  s.key.toLowerCase().includes(keyword) || s.value.toLowerCase().includes(keyword)) || 
        (product.filters || []).some(f => f.toLowerCase().includes(keyword))
      );
    });
  ...
  const generateSuggestions = (term) => {
    ...
    products.forEach(p => {
     ...
     p.specs.forEach(s => {
  if (s.key.toLowerCase().includes(keyword)) set.add(s.key);
  if (s.value.toLowerCase().includes(keyword)) set.add(s.value);
});
      (p.filters || []).forEach(f => {
        if (f.toLowerCase().includes(keyword)) set.add(f);
      });
    });
