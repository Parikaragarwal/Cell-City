...
const EditCarousel = () => {
  ...
  useEffect(() => {
    const fetchSlide = async () => {
      const doc = await appwriteService.getDocument(config.carouselCollectionId, id);
      if (doc) {
        setTitle(doc.title);
  ...
  const handleSubmit = async (e) => {
    ...
    setUpdating(true);
    const updatedData = { title, subtitle, image };
    const result = await appwriteService.updateDocument(config.carouselCollectionId, id, updatedData);
    setUpdating(false);
    if (result) {
      alert("Carousel slide updated!");
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-purple-900 flex justify-center items-center px-4">
        <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-700/50 p-8 rounded-2xl shadow-2xl">
          <div className="flex items-center justify-center">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-500 mr-3"></div>
            <span className="text-white text-lg">Loading slide data...</span>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-purple-900 flex justify-center items-center px-4 relative overflow-hidden">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm">Update your carousel content</p>
        </div>
