...
const EditElectronic = () => {
    ...
    useEffect(() => {
        const fetchElectronicData = async () => {
            try {
                const electronicData = await appwriteService.getDocument(config.electronicsCollectionId, id);
                if (electronicData) {
                    const formattedData = {
                        ...electronicData,
                        // Parse JSON strings back into arrays for the form fields
                        specs: Object.entries(JSON.parse(electronicData.specs || '{}')).map(([key, value]) => ({ key, value })),
                        features: JSON.parse(electronicData.features || '[]'),
                    };
                    reset(formattedData);
                } else {
                    navigate('/404'); // Or a dedicated error page
                }
            } catch (error) {
                console.error("Failed to fetch electronic data:", error);
            } finally {
                setIsFetching(false);
            }
        };
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const updatedDocument = await appwriteService.updateDocument(config.electronicsCollectionId, id, parsedData);
            if(updatedDocument) {
                navigate('/electronics');
            }
        } catch (error) {
            console.error("Failed to update electronic:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    if (isFetching) {
        return (
            <div className="flex justify-center items-center h-screen bg-slate-900">
                <Loader text="Loading Electronic Data..." />
            </div>
        );
    }
        ...
        <div className="relative bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 p-6 min-h-screen">
            <div className="max-w-7xl mx-auto relative z-10">
                <div className="text-center mb-8">
                    <h1 className="text-4xl font-bold text-white mb-3">Edit Electronic</h1>
                    <p className="text-slate-300">Update the details for this electronic product listing.</p>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2">
                        <form onSubmit={handleSubmit(onSubmit)} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
