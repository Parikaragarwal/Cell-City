...
useEffect(() => {
    setIsFetching(true);
    appwriteService.getDocument(config.mobilesCollectionId, id).then((mobileData) => {
        if (mobileData) {
            const specsObj = JSON.parse(mobileData.specsJSON || '{}');
const formattedData = {
    ...mobileData,
    // This now guarantees the order of the spec fields
    specs: [
        { key: "ram", value: specsObj.ram || "" },
        { key: "storage", value: specsObj.storage || "" },
        { key: "display", value: specsObj.display || "" },
    ],

    colors: JSON.parse(mobileData.colorsJSON || '[]'),
    features: JSON.parse(mobileData.featuresJSON || '[]'),
};
reset(formattedData);

        } else {
            navigate('/404'); // Or redirect to an error page
        }
...
const onSubmit = async (data) => {
    setLoading(true);
    try {
        // The parsing logic is the same as before
        const parsedData = {
            brand: data.brand,
            model: data.model,
            ...
            specsJSON: JSON.stringify(Object.fromEntries(data.specs.map(({ key, value }) => [key, value]))),
            colorsJSON: JSON.stringify(data.colors.filter((c) => c.name && c.hex)),
            featuresJSON: JSON.stringify(data.features.filter(f => f.text)),
        };

        // This is the main change: call updateDocument instead of createDocument
        const updatedDocument = await appwriteService.updateDocument(config.mobilesCollectionId, id, parsedData);

        if (updatedDocument) {
            // On success, navigate back to the products page
            navigate('/mobiles'); 
        }
    } catch (error) {
...
return (
    ...
    <div key={field.id} className="flex gap-4 items-center group/item">
        <div className="flex-1 flex gap-4 items-center">
            <input
                ...
                className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-400 focus:ring-2 focus:ring-purple-500 transition-all duration-300 backdrop-blur-sm hover:bg-white/10"
            ...
            <div className="relative">
                <input
                    ...
                    className="w-14 h-12 p-1 bg-white/5 border border-white/10 rounded-xl cursor-pointer hover:scale-110 transition-all duration-300"
                />
        ...
        <button
            ...
            className="p-3 text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded-xl transition-all duration-300 hover:scale-110 opacity-0 group-hover/item:opacity-100"
        >
            🗑️
        </button>
    </div>
    ...
    <div key={field.id} className="flex gap-4 items-center group/item">
        <input
            ...
            className="flex-1 px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-slate-400 focus:ring-2 focus:ring-emerald-500 transition-all duration-300 backdrop-blur-sm hover:bg-white/10"
        />
        <select
            {...register(`features.${index}.icon`)}
            className="px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-emerald-500 transition-all duration-300 backdrop-blur-sm hover:bg-white/10"
        >
